...
| **Testing before teaching** | Card one asked what あ reads as, to someone who had never seen あ. Every layer was correct — the queue built, FSRS scheduled, the rating saved — and the product was a guessing game whose "Lupa" was not memory data, because nothing had ever gone in | Lesson cards: a newly introduced item is taught (form, reading, meaning, sound) before any question about it, once per item, writing no review row |
| **`Number(null)` is 0, and Unit 0 exists** | `/jalur/` with no query string opened Unit 0's detail instead of the path — zero meant both "no parameter" and a real unit number | Read the parameter's presence, never its coerced value. The same trap bit BAB 0 in the LMS project; a `0` that is real data needs an explicit null check every time |
| **A unit with no resolvable items** | Unit 0 teaches kana through whole words (あさ), and those words are not items — the characters are. It resolved to nothing, so a total beginner got an empty session, and since an empty unit never passes, the path was stuck on it forever | Words expand to their kana characters when a unit declares `kana_focus`; `currentUnit` steps over a unit that resolves to nothing rather than stranding the learner on it |
